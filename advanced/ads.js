...
Ads.prototype.processAdsManager_ = function(adsManager) {
    ...
    var events = Object.values(google.ima.AdEvent.Type);
...
Ads.prototype.onAdEvent_ = function(adEvent) {
    ...
    if (adEvent.type == google.ima.AdEvent.Type.CLICK) {
        ...
        this.application_.log('Ad Pod Info, is this ad linear: ' + ad.isLinear());
        this.application_.log('Ad Pod Info, ad duration: ' + ad.getDuration());
        this.application_.log('Ad Pod Info, this ad is at position: ' + pod.getAdPosition());
        this.application_.log('Ad Pod Info, max duration: ' + pod.getMaxDuration());
        this.application_.log('Ad Pod Info, total ads ' + pod.getTotalAds());
        this.application_.log('Ad Pod Info, pod index ' + pod.getPodIndex());
        this.application_.log('Ad Pod Info, pod offset ' + pod.getTimeOffset());


        const podKey = `${pod.getPodIndex()}-${pod.getTotalAds()}-${pod.getTimeOffset()}-${pod.getMaxDuration()}`;
        if (pod.getAdPosition() == 1) {
	    var podInfo = {duration: pod.getMaxDuration(), totalAds: pod.getTotalAds(),
	    currentAdPosition: pod.getAdPosition(),
	    adDurations: [ad.getDuration()]};
	    this.podInfos[podKey] = podInfo;
		    ...
		    const podRemainingTime = (podInfo.duration - total  - podInfo.adDurations[podInfo.currentAdPosition-1]) + remainingTime;
                    // application.log('Remaining time: ' + podRemainingTime);
		    application.setAdProgress(`Ad num: ${podInfo.currentAdPosition}/${podInfo.totalAds}<br>
		    This ad: ${remainingTime}/${podInfo.adDurations[podInfo.currentAdPosition-1]}<br>
		    Pod: ${podRemainingTime}/${podInfo.duration}<br>
		    Completed ads: ${podInfo.adDurations.slice(0, podInfo.currentAdPosition-1).join(', ')}`);
		    }
                }
            }(this.adsManager_, this.application_, podInfo), 300);
            // every 300ms
        } else if (pod.getAdPosition() > 1) {
	  podInfo = this.podInfos[podKey];
	  podInfo.adDurations.push(ad.getDuration());
	  podInfo.currentAdPosition = pod.getAdPosition();
	}
	console.log(this.podInfos[podKey]);
    } else if (adEvent.type == google.ima.AdEvent.Type.COMPLETE) {
        const ad = adEvent.getAd();
        const pod = ad.getAdPodInfo();
        const podKey = `${pod.getPodIndex()}-${pod.getTotalAds()}-${pod.getTimeOffset()}-${pod.getMaxDuration()}`;
	if (pod.getTotalAds() == pod.getAdPosition()) {
	  clearInterval(this.intervals[podKey]);
	  application.setAdProgress("");
	}
    }
}
;
